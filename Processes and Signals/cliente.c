 ...
 **    ia fazer... mas neste módulo vai se armar sinais e alarmes para comunicar do cliente para os servidores dedicados criados e saber quando eles terminam, criando fifos (named pipes) no seu processo
 **
 **
 ******************************************************************************/
#include "common.h"
...
int existsFifoServidor_C1(char *nameFifo) {
    ...
    if ( stat( nameFifo, &st ) == 0 && S_ISFIFO(st.st_mode)){ //se existe e se é um fifo...
        so_success("C1", "");
        result = 0;
    }
    else {
...
int writeRequest_C5(Login request, char *nameFifo) {
    ...
    if (cliente) {
        if (fwrite(&request, sizeof(Login), 1, cliente) >= 0) {
            so_success("C5", "");
            result = 0;
            fclose(cliente);
            }
...
Login readAckLogin_C7(char *nameFifo) {
    ...
    if (ler) {
        ...
        if(fread(&ackLogin, sizeof(Login), 1, ler) < 0) {
            so_error("C7", "");
            deleteFifoAndExit_C11();
        }
        else {
...
int writeFimSessao_C10(char *nameFifo, int tempoDeProcessamento, Login ackLogin) {
    ...
    if(result == 0) {
        if(fprintf(fifo, "Sessão Login ativa durante %d segundos", tempoDeProcessamento) > 0){
            so_success("C10", "");
            fclose(fifo);
            deleteFifoAndExit_C11(); //vai para o passo C11
        }
       else {
            so_error("C10","");
            result = -1;
...
void trataSinalSIGINT_C13(int sinalRecebido) {
    ...
    if (fifo) {
        if (fprintf(fifo,"Sessão cancelada pelo utilizador") > 0){
        fclose(fifo);
        so_success("C13", "");
        }
        else {
