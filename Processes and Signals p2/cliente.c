 ...
 ** Descrição/Explicação do Módulo: este modulo serve para criar sinais e alarmes, para receber e mandar mensagens para o servidor dedicado sobre as compras dos produtos
 **
 **
 ******************************************************************************/
#include "common.h"
...
MsgContent getDadosPedidoUtilizador_C3() {
    ...
    if (msg.msgData.infoLogin.nif > 0){
...
int sendClientLogin_C4(MsgContent msg) {
    ...
    msg.msgType = MSGTYPE_LOGIN;
    int status = msgsnd(msgId, &msg, sizeof(msg.msgData), 0); //manda a msg
    if ( status < 0 ) {
        so_error("C4", "nao enviou a mensagem");
        return RET_ERROR;
    }
    else {
        so_success("C4", "enviou a mensagem");
        return RET_SUCCESS;
    }
 ...
 *        C6.2  C6.2 Valida se o resultado da autenticação do Servidor Dedicado foi sucesso ou não, 
...
int receiveProductList_C6() {
    ...
    if ( size < 0 ) {
        so_error("C6", "bad message received");
        return RET_ERROR;
    }
    else {
    ...
    //C6.5
    size = msgrcv(msgId, &msg, sizeof(msg.msgData), getpid(), 0);
    if ( size < 0 ) {
        so_error("C6.5", "bad message received");
        return RET_ERROR;
    }  
    }
    so_debug("> [@return:%d]", result);
...
int getIdProdutoUtilizador_C7() {
    ...
    printf("Digite o ID do seu produto: ");
    scanf("%d", &result);

    if (result < 0){
        so_error("C7", "%d", PRODUCT_NOT_FOUND);
        return RET_ERROR;
    }
    else {
...
int sendClientOrder_C8(int idProduto, int pidServidorDedicado) {
    ...
    int status = msgsnd(msgId, &msg, sizeof(msg.msgData), 0); //manda a msg
    if ( status < 0 ) {
        so_error("C8", "nao enviou a mensagem");
        return RET_ERROR;
    }
    else {
        so_success("C8", "enviou a mensagem");
        return RET_SUCCESS;
    }
...
void receivePurchaseAck_C9() {
    ...
    ssize_t size = msgrcv(msgId, &msg, sizeof(msg.msgData), getpid(), 0); //manda a msg
    if ( size < 0 ) {
        so_error("C9", "nao enviou a mensagem");
    }
    else {
